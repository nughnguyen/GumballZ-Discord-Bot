...
class TopCheck(commands.Cog):
    ...
    async def topcheck_enable(self, ctx):
        if ctx.author.id != ctx.guild.owner_id:
            return await ctx.reply("<:cross:1453391695705280705> Only the **Server Owner** can enable topcheck.")
        if await self.is_topcheck_enabled(ctx.guild.id):
            return await ctx.reply("<:cross:1453391695705280705> Topcheck is already enabled for this server.")
        await self.enable_topcheck(ctx.guild.id)
        await ctx.reply("<:tick:1453391589148983367> Topcheck has been Successfully enabled for this server.")
    ...
    async def topcheck_disable(self, ctx):
        ...
        if not await self.is_topcheck_enabled(ctx.guild.id):
            return await ctx.reply("<:cross:1453391695705280705> Topcheck is not enabled for this server.")
        await self.disable_topcheck(ctx.guild.id)
        await ctx.reply("<:tick:1453391589148983367> Topcheck has been Successfully disabled for this server.")
