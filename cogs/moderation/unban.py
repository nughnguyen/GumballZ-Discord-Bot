...
class Unban(commands.Cog):
    ...
    async def unban(self, ctx, user: discord.User, *, reason=None):
        ...
        try:
            await user.send(f"<:tick:1453391589148983367> You have been unbanned from **{ctx.guild.name}** by **{ctx.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:tick:1453391589148983367> | Successfully Unbaned [{user}](https://discord.com/users/{user.id})\nReason {reasonn}**", color=0xFF0000)
