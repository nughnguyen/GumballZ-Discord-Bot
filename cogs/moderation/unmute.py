...
class Unmute(commands.Cog):
    ...
    async def unmute(self, ctx, user: discord.Member):
        ...
        try:
            ...
            try:
                await user.send(f"<:tick:1453391589148983367> You have been unmuted in **{ctx.guild.name}**.")
                dm_status = "Yes"
            except discord.Forbidden:
                dm_status = "No"
            except discord.HTTPException:
            ...
            error.set_author(name="Error Unmuting User", icon_url="https://cdn.discordapp.com/emojis/1453391589148983367.png?v=1&size=48&quality=lossless")
            return await ctx.send(embed=error)

        embed = discord.Embed(
            description=f"**<:tick:1453391589148983367> | Successfully Unmuted [{user}](https://discord.com/users/{user.id})**",
            color=self.color
        )        
        embed.set_thumbnail(url=ctx.author.display_avatar.url)
