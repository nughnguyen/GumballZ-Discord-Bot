...
class RoleInfoView(View):
  ...
  @discord.ui.button(label='Show Permissions',  emoji="<:codebase:1453391605565231105> ", style=discord.ButtonStyle.secondary)
  ...
  async def show_overwrites(self, interaction: discord.Interaction, button: Button):
      ...
      for target, perms in self.channel.overwrites.items():
          ...
          overwrites.append(f"**For {target.name}**\n" +
                            "\n".join(f"  * **{perm}:** {'<:tick:1453391589148983367>>' if value else '<:cross:1453391695705280705>' if value is False else '⛔'}" for perm, value in permissions.items()))
      ...
      embed.set_footer(text="<:tick:1453391589148983367>> = Allowed, <:cross:1453391695705280705> = Denied, ⛔ = None")
      await interaction.response.send_message(embed=embed, ephemeral=True)
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"GumballZ Manager Uptime", color=self.color)
      embed.add_field(name="__UTC__", value=f"<:warning:1453394915491975414> {uptime_string}\n\n", inline=False)
      embed.add_field(name="__Online Duration__", value=f"<:uptime:1453580607312629770> {uptime_duration_string}", inline=False)
  ...
  async def serverinfo(self, ctx):
        ...
        embed.add_field(
            name="**__About__**",
            value=f"**Name : ** {ctx.guild.name}\n**ID :** {ctx.guild.id}\n**Owner <:king:1453391648003461433> :** {ctx.guild.owner} (<@{ctx.guild.owner_id}>)\n**Created At : ** {c_at}\n**Members :** {len(ctx.guild.members)}",
            inline=False
        )
        ...
        if ctx.guild.features:
            features = "\n".join([f"<:tick:1453391589148983367>>: {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}" for feature in ctx.guild.features])
            embed.add_field(
                name="**__Features__**",
        ...
        embed.add_field(
            ...
            value=f"Level: {ctx.guild.premium_tier} [<:boost:1453391600817410190>{ctx.guild.premium_subscription_count} boosts]",
            inline=False
        )
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<:tick:1453391589148983367>> Yes' if member.bot else '<:cross:1453391695705280705> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def boosts(self, ctx):
    await ctx.send(
      embed=discord.Embed(title=f"<:boost:1453391600817410190> Boosts Count Of {ctx.guild.name}",
  ...
  async def channelinfo(self, ctx, channel: discord.TextChannel = None):
    ...
    embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/1305552495942041670/1453581842027384915/1205345282158501899.png?ex=694df915&is=694ca795&hm=3c15a21f1fed30a2fa31803fdc9644b883c62cb75b2771db7538703d844b3366&")
    embed.set_footer(text=f"Requested By {ctx.author}",
                       icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
  ...
  async def report(self, ctx, *, bug):
    channel = self.bot.get_channel(1453582444312789026)
    ...
    confirm_embed = discord.Embed(title="<:tick:1453391589148983367> Bug Reported",
