...
class Status(commands.Cog):
    ...
    async def status(self, ctx, user: discord.User = None):
        ...
        processing = await ctx.send("<a:loading:1453413858881306684> Loading Status...")
        embed = discord.Embed(title=f"{user.display_name}'s Status", color=0xFF0000)

        status_emoji = {
            "online": "<a:online:1453413871728726157> Online",
            "idle": "<:idle:1453391642789806080> Idle",
            "dnd": "<:dnd:1453391615103209582> Do Not Disturb",
            "offline": "<:offline:1453391933845274745>  Offline"
        }
        ...
        if member:
            status = status_emoji.get(str(member.status), "<:offline:1453391933845274745> Offline")
            ...
            try:
                ...
                embed.add_field(name="Status:", value="<:offline:1453391933845274745> Offline", inline=False)
    ...
    def get_platform(self, member):
        if member.desktop_status != discord.Status.offline:
            return "<:pc:1453391656475951124>  Desktop"
        elif member.mobile_status != discord.Status.offline:
            return "<a:mobile:1453413865416298673> Mobile"
        elif member.web_status != discord.Status.offline:
            return "<:icon_browser:1453391640298389554> Browser"
        return "Unknown"
