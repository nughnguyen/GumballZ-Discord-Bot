...
class AutoResponder(commands.Cog):
    ...
    async def _create(self, ctx, name, *, message):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT COUNT(*) FROM autoresponses WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                ...
                if count >= 20:
                    return await ctx.reply(embed=discord.Embed(title="<:cross:1453391695705280705> Error!",
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:tick:1453391589148983367> Success",
    ...
    async def _config(self, ctx):
        ...
        embed = discord.Embed(color=0xFF0000, title=f"Autoresponders in {ctx.guild.name}")
