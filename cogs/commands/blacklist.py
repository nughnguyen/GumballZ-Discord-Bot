...
class BlacklistWordSource(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        ...
        embed.set_footer(
            ...
            icon_url="https://media.discordapp.net/attachments/1305552495942041670/1305553118586601523/aa4804fba8b3f81c6e75e5ef2888fec2.jpg?ex=694d5697&is=694c0517&hm=5e52896950c980631457ea0914bb381670772a4289b6525d83777ac6d7092390&=&format=webp&width=720&height=800"
        )
        return embed
...
############ FUNCTIONS ############
    ...
    async def add(self, ctx, word: str):
        ...
        if await self.is_word_blacklisted(guild_id, word.lower()):
            embed = discord.Embed(title="<:tick:1453391589148983367> Access Denied",
    ...
    async def bypass_add(self, ctx, target: Union[discord.Member, discord.Role]):
        ...
        if isinstance(target, discord.Member):
            ...
            if await self.is_user_bypassed(guild_id, target.id):
                embed = discord.Embed(
                    description=f"<:cross:1453391695705280705> | `{target}` is already bypassed.",
                    color=discord.Color.from_rgb(255, 0, 0)
                )
                await ctx.reply(embed=embed)
                return
    ...
    async def command_error(self, ctx, error):
        if isinstance(error, commands.CommandError):
            if not isinstance(error, commands.CommandOnCooldown):
                embed = discord.Embed(
                    description="<:cross:1453391695705280705> | An error occurred while processing the command. Make sure you have **Administrator** permission.",
                    color=discord.Color.from_rgb(255, 0, 0)
                )
                await ctx.reply(embed=embed)