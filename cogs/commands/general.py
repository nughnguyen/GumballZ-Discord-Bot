...
class General(commands.Cog):
  ...
  async def poll(self, ctx: commands.Context, *, message):
    ...
    await msg.add_reaction("<:tick:1453391589148983367>")
    await msg.add_reaction("<:cross:1453391695705280705>")
  ...
  async def wizz(self, ctx: commands.Context):
    ...
    embed = discord.Embed(
      ...
      description=f"**<:warning:1453394915491975414> Successfully Wizzed {ctx.guild.name}**",
      color=self.color,
  ...
  async def invite(self, ctx: commands.Context):
      embed = discord.Embed(
          title="<:connection:1453394939730985162> GumballZ Integration Hub!",
          description="```Empower your server with blazing-fast features and 24/7 support!```",
          color=0xFF0000
      )
      embed.set_thumbnail(url=self.bot.user.avatar.url)
      embed.add_field(
          name="<:links:1453394953941024798> Quick Actions",
          value=(
              ">>> **[Invite GumballZ](https://discord.com/oauth2/authorize?client_id=1305035261897343026&permissions=8&integration_type=0&scope=bot+applications.commands)**\n"
              "**[Support Server](https://dsc.gg/thenoicez)**"
          ),
          inline=False
      )
      embed.set_footer(
          text=f"Powerd By GumballZ",
