    ...
    async def add(self, ctx, role: discord.Role):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            embed = discord.Embed(title="<:tick:1453391589148983367> Success",
                                  description=f"VC role {role.mention} added for this guild.", color=0xFF0000)
            await ctx.reply(embed=embed)
            ...
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id)) as cursor:
                ...
                if not row:
                    embed = discord.Embed(title="<:cross:1453391695705280705> Error",
                                          description="Given role is not set in VC role.", color=0xFF0000)
                    await ctx.send(embed=embed)
                    return
            await db.execute('DELETE FROM vcroles WHERE guild_id = ? AND role_id = ?', (ctx.guild.id, role.id))
            await db.commit()
            embed = discord.Embed(title="<:tick:1453391589148983367> Success",
                                  description=f"VC role {role.mention} removed for this guild.", color=0xFF0000)
            await ctx.send(embed=embed)
    ...
    async def config(self, ctx):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (ctx.guild.id,)) as cursor:
                ...
                if not row:
                    embed = discord.Embed(title="<:cross:1453391695705280705> Error",
                                          description="VC role is not set in this guild.", color=0xFF0000)
                    await ctx.send(embed=embed)
                    return
                role = ctx.guild.get_role(row[0])
                embed = discord.Embed(title="VC Role Configuration",
                                      description=f"Current VC role in this guild is {role.mention}.", color=0xFF0000)
            ...
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (member.guild.id,)) as cursor:
                    ...
                    if after.channel and role not in member.roles:
                        await self.add_role_with_retry(member, role, reason="Member Joined VC | GumballZ Invcrole")
                    elif not after.channel and role in member.roles:
                        await self.remove_role_with_retry(member, role, reason="Member Left VC | GumballZ Invcrole")
