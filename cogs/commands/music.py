...
class MusicControlView(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        await interaction.response.send_message(
            embed=discord.Embed(description="Only members in the same voice channel as me can control the player.", color=0xFF0000),
            ephemeral=True
        )
        return False

    @discord.ui.button(emoji="<:unmute:1453391593917907147>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:rewind:1453652995991404584>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:music_stop:1453653783698608340>", style=discord.ButtonStyle.success)
    async def pause_button(self, interaction: discord.Interaction, button: Button):
        if self.player.paused:
            await self.player.pause(False)

            await self.player.channel.edit(status=f"<:icons_pause:1453657906720542720> Playing: {self.player.current.title}")
            button.emoji = "<:music_stop:1453653783698608340>" 
            await interaction.response.edit_message(view=self)

        elif self.player.playing:
            await self.player.pause(True)
            await self.player.channel.edit(status=f"<:icons_pause:1453657906720542720>  Paused: {self.player.current.title}")
            button.emoji = "<:icons_next:1453658490248626258> "
            await interaction.response.edit_message(view=self)


    @discord.ui.button(emoji="<:skip:1453660548158849088>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:iconLoad:1453663471370768517>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:shuffle:1453663432413941934> ", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:music_stop:1453653783698608340>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:forward:1453664212768260187>", style=discord.ButtonStyle.secondary)
    ...
    @discord.ui.button(emoji="<:icons_music:1453667388942979113> ", style=discord.ButtonStyle.secondary)
...
class Music(commands.Cog):
        ...
        if len(guild.voice_channels[0].members) == 1:
            ...
            if player:
                ...
                try:
                    ...
                    ended.set_footer(text="Thanks for choosing GumballZ!")
                    support = Button(label='Support',
                                 style=discord.ButtonStyle.link,
                        url=f'https://dsc.gg/thenoicez')
                    vote = Button(label='Vote',
                                 style=discord.ButtonStyle.link,
                        url=f'https://top.gg/bot/1305035261897343026/vote')
                    view = View()
    ...
    async def display_player_embed(self, player, track, ctx, autoplay=False):
        if track.artwork:
            ...
            embed.set_author(name="Now Playing", icon_url="https://cdn.discordapp.com/attachments/1305556786304127097/1453668557480263680/1275556609958875218.gif?ex=694e49d7&is=694cf857&hm=19fff33ef2c4dfa73b3979dff3f3a678cce670e24122921a8975d89ff7d56c03&")
            embed.add_field(name="Author", value=f"`{track.author}`")
            embed.add_field(name="Duration", value=f"`{duration}`")
            embed.add_field(name="Source", value=f"[<:spotify:1453670470028361741> Listen on Spotify]({track.uri})" if "spotify" in track.source else f"[<:jiosaavn:1453670462180560936> Listen on JioSaavn]({track.uri})" if "jiosaavn" in track.source else f"[<:soundcloud:1453670465359974430> Listen on SoundCloud]({track.uri})" if "soundcloud" in track.source else f"[<:youtubelogo:1453670472402341960> Listen on YouTube]({track.uri})")
    ...
    async def play_source(self, ctx, query):
        if not ctx.author.voice:
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> you need to be in a voice channel to use this command.", color=0xFF0000))
            return
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.send(embed=discord.Embed(description=f"<:plus:1453391565904019579> Added playlist [{tracks.name}](https://dsc.gg/thenoicez) with **{len(tracks.tracks)} songs** to the queue.", color=0xFF0000))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(embed=discord.Embed(description=f"<:plus:1453391565904019579> Added [{track.title}](https://dsc.gg/thenoicez) to the queue.", color=0xFF0000))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(embed=discord.Embed(description=f"<:plus:1453391565904019579> Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://dsc.gg/thenoicez)** to the queue.", color=0xFF0000))
                await lmao.delete()
                ...
                await ctx.send(embed=discord.Embed(description=f"<:plus:1453391565904019579> Added all tracks from album **[{album_info['name']}](https://dsc.gg/thenoicez)** to the queue.", color=0xFF0000))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
    ...
    async def autoplay(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> No song is currently playing.", color=0xFF0000))
            return

        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> You need to be in the same voice channel as me to use this command.", color=0xFF0000))
            return
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:tick:1453391589148983367> Autoplay {'enabled' if vc.autoplay == wavelink.AutoPlayMode.enabled else 'disabled'} by {ctx.author.mention}.", color=0xFF0000))
    ...
    async def loop(self, ctx: commands.Context):
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:tick:1453391589148983367> Loop {'enabled' if vc.queue.mode == wavelink.QueueMode.loop else 'disabled'} by {ctx.author.mention}.", color=0xFF0000))
        else:
    ...
    async def pause(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> No song is currently playing.", color=0x000000))
            return
        ...
        if vc and vc.playing and not vc.paused:
            ...
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712>   Nothing is playing or already paused.", color=0xFF0000))
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1453394993137057939> You need to be in the same voice channel as me to use this command.", color=0xFF0000))
            return

        if vc and vc.paused:
            await vc.pause(False)
            await vc.channel.edit(status=f"<a:music:1453413868775673990> Playing: {vc.current.title}")
            await ctx.send(embed=discord.Embed(description=f"Resumed by {ctx.author.mention}.", color=0xFF0000))
        else:
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> Player is not paused.", color=0xFF0000))
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.queue.is_empty:
            ...
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> No song is playing or in the queue to skip.", color=0xFF0000))
    ...
    async def volume(self, ctx: commands.Context, level: int):
        ...
        if not vc:
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> I'm not connected to a voice channel.", color=0xFF0000))
            return
        ...
        if vc:
            if 1 <= level <= 150:
                ...
                await ctx.send(embed=discord.Embed(description=f"<:mute:1453391560271204362> Volume set to {level}% by {ctx.author.mention}.", color=0xFF0000))
            else:
                await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> Volume must be between 1 and 150.", color=0xFF0000))
        else:
    ...
    async def queue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> The queue is currently empty.", color=0xFF0000))
            return
    ...
    async def clearqueue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> No Queue to clear.", color=0xFF0000))
            return
    ...
    async def replay(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:warning:1453391673743638712> I'm not connected to any voice channel.", color=0xFF0000))
            return
    ...
    async def disconnect(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc:
            await ctx.send(embed=discord.Embed(description="<:icons_warning:1453394993137057939>  I'm not connected to any voice channel.", color=0xFF0000))
            return
    ...
    async def on_wavelink_track_start(self, payload: wavelink.TrackStartEventPayload):
        ...
        if voice_channel:
            await voice_channel.edit(status=f"<a:music:1453413868775673990> Playing: {track.title}")  # type: ignore
