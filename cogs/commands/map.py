...
class MapView(ui.View):
    ...
    def get_coordinates(self, location):
        ...
        try:
            headers = {'User-Agent': 'GumballZ Bot (https://gumballzhub.vercel.app)'}
    ...
    async def update_embed(self, interaction: discord.Interaction):
        if self.latitude is None or self.longitude is None:
            ...
            return
        embed = discord.Embed(title=f" Map of {self.location}", color=0xFF0000)
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user != self.ctx.author:
            await interaction.response.send_message(
                embed=discord.Embed(description="Sorry only the requested author can control this", color=0xFF0000),
                ephemeral=True
            )
            return False
        return True
    ...
    @discord.ui.button(label="", emoji="<:delete:1453602008786534514>", style=ButtonStyle.danger)
