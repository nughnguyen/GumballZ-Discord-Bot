...
from core import gumballz, Cog, Context
...
class Errors(Cog):
  ...
  async def on_command_error(self, ctx: Context, error):
    ...
    if isinstance(error, commands.CommandOnCooldown):
      embed = discord.Embed(color=0xFF0000, description=f"**{ctx.author.mention} Cooldown is here Bro Try commands in {error.retry_after:.2f} seconds**.")
